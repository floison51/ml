...
​
    ...
    x = np.zeros( ( vocab_size, n_a ), dtype=np.int )
    # Step 1': Initialize a_prev as zeros (≈1 line)
    a_prev = np.zeros( ( n_a, n_a ), dtype=np.int )
    ...
    while (idx != newline_character and counter != 50):
        ...
        y_flat = y[ :, counter ].ravel()
        idx = np.random.choice( np.arange( vocab_size ), p = y_flat )
...
def test_sample( vocab_size, char_to_ix, ix_to_char ):
