...
def compute_cost(Z3, Y, beta, parameters ):
    ...
    raw_cost = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits = logits, labels = labels))
...
def model( 
    nbUnits, X_train, Y_train, X_test, Y_test, 
    learning_rate = 0.0001, beta = 0, keep_prob = [1,1,1],
    num_epochs = 1500, minibatch_size = 32, print_cost = True, show_plot = True):
    """
    ...
    with tf.Session() as sess:
        ...
        paramsFileName = "saved/params-Beta" + str( beta ) + ".bin"
...
def model_batch( 
    ...
    model( nbUnits, X_train, Y_train, X_test, Y_test, learning_rate, 
           beta, keep_prob,
          num_epochs, minibatch_size, print_cost = True, show_plot = False)
...
if __name__ == '__main__':
    ...
    parameters = model_batch( 
        nbUnits, X_train, Y_train, X_test, Y_test, 
        beta=0, keep_prob = keep_prob 
    )
